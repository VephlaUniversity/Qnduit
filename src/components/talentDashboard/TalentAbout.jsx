...
const TalentAbout = () => {
  ...
  const handleAvatarUpload = (e) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      toast({ title: "Upload Error", description: "File exceeds 5MB limit", variant: "destructive" });
      return;
    }
  ...
  const inputClass = "w-full px-4 py-3 bg-[#0E0E10] border border-white/10 rounded-lg text-white focus:outline-none focus:border-blue-500";
  const selectClass = "w-full px-4 py-3 bg-[#0E0E10] border border-white/10 rounded-lg text-white focus:outline-none focus:border-blue-500";
  ...
  const socialIcons = [
    ...
    { name: "instagram", icon: Instagram, placeholder: "http://www.instagram.com" },
  ...
  return (
    <div className="min-h-screen bg-[#0E0E10] p-4 md:p-6 lg:p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl md:text-3xl font-semibold text-white">Profile Setting</h1>
      </div>
      ...
      <div className="bg-[#1A1A1E] rounded-lg border border-white/5 p-6 md:p-8 max-w-5xl">
        ...
        <div className="flex items-start gap-6 mb-8">
          ...
          <div className="flex-1">
            <label className="block text-white font-medium mb-2">Upload a new avatar:</label>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => avatarInputRef.current?.click()} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm transition-colors">
                Choose File
              </button>
          ...
          <button onClick={handleSave} disabled={saving} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors disabled:opacity-70 flex items-center gap-2">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select name="age" value={formData.age} onChange={handleInputChange} className={selectClass}>
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">Rate Card ($)</label>
              <input type="text" name="offeredSalary" value={formData.offeredSalary} onChange={handleInputChange} className={inputClass} />
            </div>
           <div>
  <label className="block text-gray-400 text-sm mb-2">Salary Type</label>
  <select name="salaryType" value={formData.salaryType} onChange={handleInputChange} className={selectClass}>
    ...
    <option>Project</option>
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">Experience time</label>
              <select name="experienceTime" value={formData.experienceTime} onChange={handleInputChange} className={selectClass}>
  ...
  />
            ...
            <div>
              ...
              <input type="text" name="jobTitle" value={formData.jobTitle} onChange={handleInputChange} className={inputClass} />
            </div>
            <div>
              <label className="block text-gray-400 text-sm mb-2">Categories</label>
              <input type="text" name="categories" value={formData.categories} onChange={handleInputChange} className={inputClass} />
            </div>
          </div>

          {/* Show my profile */}
          <div className="mt-6">
            <label className="block text-gray-400 text-sm mb-3">Show my profile</label>
            <div className="flex gap-6">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="showProfile" value="show" checked={formData.showProfile === "show"} onChange={handleInputChange} className="w-4 h-4 accent-blue-600" />
                <span className="text-white">Show</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="showProfile" value="hidden" checked={formData.showProfile === "hidden"} onChange={handleInputChange} className="w-4 h-4 accent-blue-600" />
          ...
          {/* Tag Skill */}
          <div className="mt-6">
            <label className="block text-gray-400 text-sm mb-3">Tag Skill</label>
            <div className="flex flex-wrap items-center gap-2 w-full px-4 py-3 bg-[#0E0E10] border border-white/10 rounded-lg min-h-[48px]">
              {tags.map((tag, index) => (
                <span key={index} className="px-3 py-1 bg-blue-600 text-white rounded-md flex items-center gap-2 text-sm">
                  {tag}
                  <button onClick={() => handleRemoveTag(tag)} className="hover:text-gray-300 transition-colors">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
        ...
        {/* Social Network */}
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-white mb-4">Social Network</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              return (
                <div key={social.name} className="flex gap-3">
                  ...
                  {formData[social.name] && (
                    <button
                      onClick={() => setFormData((prev) => ({ ...prev, [social.name]: "" }))}
        ...
        <div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input type="text" name="introVideo" value={formData.introVideo} onChange={handleInputChange} placeholder="https://www.youtube.com/watch?v=..." className={inputClass} />
            </div>
          </div>
        </div>
      </div>
