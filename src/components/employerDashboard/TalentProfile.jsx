...
// Skill progress
const getSkillProgress = (candidateId, index) => {
  const seed = (candidateId * 7 + index * 13) % 40;
  return 60 + seed;
};
...
const ProfilePage = ({ candidate, onBack }) => {
  ...
  const sidebarFields = [
    ...
    { label: "Experience", value: candidate.experienceTime },
          ...
          >
            <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
              <div className="flex flex-col gap-4 sm:flex-row sm:gap-6">
                {/* Avatar */}
                <div className="w-20 h-20 md:w-24 md:h-24 bg-blue-600 rounded-lg flex-shrink-0 flex items-center justify-center text-white text-3xl font-bold">
                  ...
                  <div className="flex gap-4 md:gap-8 min-w-max">
                    {["resume", "portfolio", "contact"].map((tab) => (
                      <button
                        ...
                        className={`pb-4 font-medium capitalize text-sm md:text-base ${
                          activeTab === tab
                ...
                <AnimatePresence mode="wait">
                  ...
                  {activeTab === "resume" && (
                    ...
                    >
                      ...
                      {/* Skills — dynamically rendered from candidate.skills */}
                      {candidate.skills?.length > 0 && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
                          {[skillGroupA, skillGroupB].map(
                            (group, groupIdx) =>
                              group.length > 0 && (
                                <div key={groupIdx}>
                    ...
                    >
                      ...
                      <div className="relative">
                        <button
                          ...
                          className="absolute left-0 top-1/2 -translate-y-1/2 w-8 h-8 bg-white rounded-full flex items-center justify-center hover:bg-gray-100 z-10"
                        >
                          <ChevronLeft className="w-4 h-4 text-gray-800" />
                        </button>
                        <div className="grid grid-cols-3 gap-2 md:gap-4 px-10">
                        ...
                        <button
                          ...
                          className="absolute right-0 top-1/2 -translate-y-1/2 w-8 h-8 bg-white rounded-full flex items-center justify-center hover:bg-gray-100 z-10"
                        >
                    ...
                    >
                      ...
                      >
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              type="text"
                              placeholder="Subject"
                              className="w-full bg-[#0F1419] border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 text-sm"
                            />
                          </div>
                          <div>
                            <label className="block text-gray-300 mb-2 text-sm">
                            ...
                            <input
                              ...
                              className="w-full bg-[#0F1419] border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 text-sm"
                            />
                          </div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <textarea
                            rows={6}
                            placeholder={`Write a message to ${candidate.name.split(" ")[0]}...`}
                            className="w-full bg-[#0F1419] border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 resize-none text-sm"
                          />
                        </div>
                        <button
                          type="submit"
                          className="bg-blue-500 hover:bg-blue-600 text-white px-8 py-3 rounded-lg font-medium transition-colors text-sm w-full sm:w-auto"
                        >
                          Send Message
                        </button>
                      </form>
            ...
            >
              <div className="bg-[#191D23] rounded-2xl p-5 md:p-6 space-y-6">
                ...
                <div className="space-y-1">
                  {sidebarFields.map(({ label, value }, i) =>
                    value ? (
                      <motion.div
                        key={label}
                  ...
                  >
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {[
                        ...
                        <button
                          key={idx}
                          className="w-9 h-9 bg-[#0F1419] rounded flex items-center justify-center hover:bg-blue-500 transition-colors"
                        >
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <p className="text-xs text-gray-400">
                        CV_{candidate.name.replace(" ", "_").toUpperCase()}
                      </p>
                      <p className="text-white font-medium text-sm">PDF</p>
                    </div>
                  </div>
                  <button className="w-full bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-lg font-medium flex items-center justify-center gap-2 text-sm">
                    <Download className="w-4 h-4" />
                    Download CV
                  </button>
                </motion.div>
